...
export default function FolderModal({ open, onClose, onSubmit, modalTitle = "Create Reminder Template", isSubmitting = false }) {
  ...
  const handleSubmit = async () => {
    ...
    if (!normalizedTemplate.length) {
      setSubmitError("Add at least one valid template field before creating the reminder.");
      return;
    }
      ...
      <div className="space-y-5">
        ...
        <label className="block space-y-2">
          ...
          <input
            ...
            placeholder="Daily Task Reminder"
            type="text"
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <div className="text-xs text-slate-500">These fields are sent as the reminder `template` array.</div>
            </div>
            <button
          ...
          <div className="max-h-[48vh] space-y-4 overflow-y-auto pr-1">
            {formState.template.map((field, fieldIndex) => (
              <div key={`reminder-template-field-${fieldIndex}`} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
