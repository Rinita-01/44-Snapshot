...
export default function DynamicEntryForm({ open, folderName, template, onClose, onSubmit, isSubmitting = false, initialSections }) {
  ...
  const [sections, setSections] = useState(() => {
    if (Array.isArray(initialSections) && initialSections.length) {
      return initialSections.map((section) => ({ ...emptySection, ...section }));
    }
  ...
  React.useEffect(() => {
    if (Array.isArray(initialSections) && initialSections.length) {
      setSections(initialSections.map((section) => ({ ...emptySection, ...section })));
      return;
    }

    setSections([{ ...emptySection }]);
  }, [emptySection, initialSections]);
  ...
  if (open === undefined) {
    return (
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="mb-5">
          <h2 className="text-xl font-semibold text-slate-900">{folderName} Form</h2>
          <p className="mt-1 text-sm text-slate-500">This form uses the fields configured when the folder was created.</p>
        </div>
